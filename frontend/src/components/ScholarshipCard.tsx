...
const ScholarshipCard: React.FC<ScholarshipCardProps> = ({
  ...
  const getTypeBadge = (type: ScholarshipType) => {
    switch (type) {
      case ScholarshipType.GOVERNMENT:
        return 'badge-warning';
      case ScholarshipType.THESIS_GRANT:
        return 'badge-info';
      case ScholarshipType.UNIVERSITY:
        return 'badge-success';
      case ScholarshipType.PRIVATE:
        return 'badge-secondary';
      case ScholarshipType.COLLEGE:
            ...
            <ul className="space-y-1">
              ...
              {matchResult.eligibilityDetails.filter((d: any) => !d.passed).length > 3 && (
                <li className="text-sm text-red-500 italic">
                  +{matchResult.eligibilityDetails.filter((d: any) => !d.passed).length - 3} more criteria
                </li>
              )}
            </ul>
          </div>
