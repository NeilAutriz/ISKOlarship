...
import { Scholarship, MatchResult, EligibilityCheckResult } from '../types';
...
const ScholarshipDetails: React.FC = () => {
  ...
  // Get match result if user is logged in
  const matchResult = useMemo(() => {
    if (!user || !scholarship) return null;
    const results = matchStudentToScholarships(user, [scholarship]);
    return results.length > 0 ? results[0] : null;
  }, [user, scholarship]);

  // Get prediction details if eligible
  const prediction = useMemo(() => {
    if (!user || !scholarship || !matchResult?.isEligible) return null;
    return predictScholarshipSuccess(user, scholarship);
  }, [user, scholarship, matchResult]);
      ...
      <div className="container-app py-8">
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {scholarship.description && (
              <div className="card p-6">
                <h2 className="text-lg font-semibold text-slate-900 mb-4 flex items-center gap-2">
                  <Info className="w-5 h-5 text-uplb-600" />
            ...
            <div className="card p-6">
              <h2 className="text-lg font-semibold text-slate-900 mb-4 flex items-center gap-2">
                <CheckCircle className="w-5 h-5 text-uplb-600" />
                Eligibility Requirements
              </h2>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                {scholarship.eligibilityCriteria?.minGWA && (
                  <div className={`p-4 rounded-xl border ${
                    matchResult
                      ? user && user.gwa >= scholarship.eligibilityCriteria.minGWA
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {matchResult && user && (
                        user.gwa >= scholarship.eligibilityCriteria.minGWA
                ...
                {scholarship.eligibilityCriteria?.requiredYearLevels && scholarship.eligibilityCriteria.requiredYearLevels.length > 0 && (
                  <div className={`p-4 rounded-xl border ${
                    matchResult
                      ? scholarship.eligibilityCriteria.requiredYearLevels.includes(user?.yearLevel!)
                        ? 'bg-green-50 border-green-200'
                        : 'bg-red-50 border-red-200'
                      : 'bg-slate-50 border-slate-200'
                  }`}>
                ...
                {scholarship.eligibilityCriteria?.eligibleColleges && scholarship.eligibilityCriteria.eligibleColleges.length > 0 && (
                  <div className={`p-4 rounded-xl border ${
                    matchResult
                      ? scholarship.eligibilityCriteria.eligibleColleges.includes(user?.college!)
                        ? 'bg-green-50 border-green-200'
                        : 'bg-red-50 border-red-200'
                      : 'bg-slate-50 border-slate-200'
                  }`}>
                ...
                {scholarship.eligibilityCriteria?.maxAnnualFamilyIncome && (
                  <div className={`p-4 rounded-xl border ${
                    matchResult
                      ? user && user.annualFamilyIncome <= scholarship.eligibilityCriteria.maxAnnualFamilyIncome
                        ? 'bg-green-50 border-green-200'
                        : 'bg-red-50 border-red-200'
                      : 'bg-slate-50 border-slate-200'
                  }`}>
                    <div className="flex items-center gap-3">
                      ...
                      {matchResult && user && (
                        user.annualFamilyIncome <= scholarship.eligibilityCriteria.maxAnnualFamilyIncome
                          ? <CheckCircle className="w-5 h-5 text-green-600 ml-auto" />
                          : <XCircle className="w-5 h-5 text-red-600 ml-auto" />
                      )}
                    </div>
            ...
            {prediction && (
              <div className="card p-6">
                <h2 className="text-lg font-semibold text-slate-900 mb-4 flex items-center gap-2">
                  <BarChart2 className="w-5 h-5 text-uplb-600" />
                ...
                <div className="mb-6">
                  ...
                  <p className="text-sm text-slate-500 mt-2">
                    Confidence: {(prediction.confidence * 100).toFixed(0)}% • {prediction.recommendation}
                  </p>
                </div>
          ...
          <div className="space-y-6">
            ...
            <div className="card p-6">
              ...
              <div className="space-y-3">
                {user ? (
                  ...
                  <Link to="/?login=true" className="btn-primary w-full flex items-center justify-center gap-2">
                    Sign In to Apply
                  </Link>
                )}
            ...
            <div className="card p-6">
              ...
              <a
                ...
                className="text-uplb-600 text-sm font-medium inline-flex items-center gap-1 hover:text-uplb-700"
              >
                Visit OSG Website
