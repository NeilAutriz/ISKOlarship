...
export const filterScholarships = (
  ...
  return scholarships.filter(scholarship => {
    ...
    // Deadline filter (only show active scholarships)
    if (criteria.hideExpired && scholarship.applicationDeadline) {
      const now = new Date();
      if (new Date(scholarship.applicationDeadline) < now) return false;
    }
