...
export const calculateEligibilityScore = (
  ...
  // GWA check
  if (criteria.minimumGWA !== undefined) {
    totalCriteria++;
    if (student.gwa <= criteria.minimumGWA) {
      // Perfect match if student GWA is significantly better
      const gwaDiff = criteria.minimumGWA - student.gwa;
      score += Math.min(1, 0.5 + gwaDiff * 0.5);
    }
  }

  // Year level check
  if (criteria.allowedYearLevels && criteria.allowedYearLevels.length > 0) {
    totalCriteria++;
    if (criteria.allowedYearLevels.includes(student.yearLevel)) {
      score += 1;
    }
  }

  // College check
  if (criteria.allowedColleges && criteria.allowedColleges.length > 0) {
    totalCriteria++;
    if (criteria.allowedColleges.includes(student.college)) {
      score += 1;
    }
  }

  // Income check
  if (criteria.maximumAnnualIncome !== undefined) {
    totalCriteria++;
    if (student.annualFamilyIncome <= criteria.maximumAnnualIncome) {
      // Better score for lower income (more need)
      const incomeRatio = student.annualFamilyIncome / criteria.maximumAnnualIncome;
